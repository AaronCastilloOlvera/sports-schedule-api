...
`routes/mlb_radar.py` — MLB Radar: motor de picks de béisbol (MLB + LMB). Endpoints: `GET /mlb-radar/suggestions?date=&league=`, `GET /mlb-radar/cached?date=&league=`, `GET /mlb-radar/accuracy?days=&min_confidence=&league=&date=`. Lógica en `services/mlb_radar_service.py`, worker nocturno en `tasks/prewarm_mlb_radar.py`. **Redis-only, sin tablas ni migraciones** — `statsapi.mlb.com` es gratuita y sin cuota, así que re-consultar historia no cuesta nada. Cuatro mercados: `moneyline`, `nrfi`, `total` (líneas fijas 7.5/8.5/9.5), `hits` (líneas fijas 4.5/5.5/6.5/7.5, un pick por abridor). Las líneas de `total` y `hits` se eligen por cercanía a la expectativa neutral, **nunca** desde la proyección propia — elegirla desde la proyección garantiza caer del lado cómodo y produce hit rates ficticios. La expectativa neutral de `total`/`hits` también se ajusta por día de la semana y franja horaria (`compute_day_time_multipliers()`, espejo de `_compute_dow_multipliers()` de fútbol): un multiplicador por día específico (0=Lun..6=Dom, sin categoría aparte de entre-semana/fin-de-semana — el efecto de fin de semana ya lo capturan sáb/dom) y otro por franja horaria LOCAL del estadio (`day` <15:00, `afternoon` 15:00–18:00, `evening` 18:00+), gateados por `DOW_MIN_SAMPLE` igual que park factor. La hora local resuelve las 4 zonas horarias de EEUU vía `MLBApiClient.get_venue_timezone()` (IANA tz id real desde `statsapi.mlb.com`, no una tabla estadio→zona hardcodeada). Nunca se aplica a `moneyline`/`nrfi` (mercados de probabilidad, no de valor proyectado vs línea). Confianza topada por tamaño de muestra (`CONFIDENCE_CAPS`); nunca emite 100. Todo pick lleva `odd: None` — no existe fuente de momios para MLB.
...
| `mlb:venue_tz:{venue_id}` | 180 d | `MLBRadarService` |
